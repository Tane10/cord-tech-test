...
SchemaValidation.prototype.validateParams = function (
  allParams,
  requiredParams
) {
  /**
   * all we care about when checkin the param is
   * - in -> "body" -> "query"
   * - if query -> name, type ->
   *    Object.keys(retrieveRequestDetails.queryString).forEach() if key == name then type === typeof retrieveRequestDetails.queryString.entries[]
   * - if body -> require
   *
   * either way query sting and body from the REQUEST are in the shape of objects
   *
   *
   */
};
...
SchemaValidation.prototype.validate = function () {
  ...
  const validRequiredParameters = this.validateParams(
    parameters,
    requiredParameters
  );
};
